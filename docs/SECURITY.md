...
- remote workers can also pair with a short-lived pairing token generated by the control plane
- remote workers receive a token on registration
- only the token hash is stored
- invalid worker auth and worker state changes are audited
- remote workers only receive work assigned by the server; they do not self-assign arbitrary jobs
