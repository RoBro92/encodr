...
class Job(Base, IdMixin, TimestampMixin):
    ...
    cleared_reason: Mapped[str | None] = mapped_column(String(255), index=True)
    ...
    backup_policy: Mapped[str] = mapped_column(String(64), nullable=False, default="keep")
    backup_retention_until: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), index=True)
    backup_deleted_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    backup_restored_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
